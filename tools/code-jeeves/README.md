...
- change the structure of tasks so that they represent a dependency tree
  - <del>traverse through the tree from the leaves and create the functions</del>
  - <del>if all leaves of a parent node succeed then create the function for this node, with the dependencies of the leaves</del>
  - <del>if not all leaves succeed then go to the "next parents leaves"</del>
...
- <del>tab completion with yargs (oclif seems too much overhead)</del>
