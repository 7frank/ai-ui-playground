...
export async function handleDocumentation({
    ...
    const found = await $`find . -type d -name node_modules -prune -o -type f -name "${pattern}"`.text();
    const files = found.split("\n");

    const selectedFile = files.length == 1 ? files[0] : (await fileSelectQuestion(files)).fileName;
    ...
    if (await confirmQuestion("Do you want to commit changes?")) {
        const commitSuccess = await $`git commit -m ${commitMessage} ${dryRun}`.text();
